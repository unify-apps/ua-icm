# Entity type (object) snapshots (tool)

Rebuilt automatically by `scripts/ua.mjs snap-types`. One JSON per object type.

| id | name | tags | schema fields |
| --- | --- | --- | --- |
| AttainmentMeasure | AttainmentMeasure | icm | 4 |
| AuditLog | AuditLog | icm | 9 |
| AutomationTemplate | AutomationTemplate | icm | 7 |
| CalculationRun | CalculationRun | icm | 10 |
| Credit | Credit | icm | 10 |
| CreditRule | CreditRule | icm | 9 |
| CreditType | Credit Type | icm | 4 |
| Currency | Currency | icm | 6 |
| DataSync | DataSync | icm | 19 |
| DataSyncProbeShape | DataSyncProbeShape | icm | 3 |
| DataSyncRun | DataSyncRun | icm | 19 |
| DataSyncRunError | DataSyncRunError | icm | 12 |
| DataSyncSample | DataSyncSample | icm | 6 |
| DataSyncUiTest | DataSyncUiTest | icm | 5 |
| DataSyncUiTest2 | DataSyncUiTest2 | icm | 4 |
| DataSyncUiTest3 | DataSyncUiTest3 | icm | 4 |
| Dispute | Dispute | icm | 12 |
| Earning | Earning | icm | 20 |
| FxRate | FxRate | icm | 5 |
| IngestTest | IngestTest | icm | 6 |
| IngestTest2 | IngestTest2 | icm | 6 |
| IngestTest3 | IngestTest3 | icm | 6 |
| IngestTest4 | IngestTest4 | icm | 6 |
| Leave | Leave | icm | 8 |
| MeasureResult | MeasureResult | icm | 10 |
| NotificationLog | NotificationLog | icm | 7 |
| Payee | People | icm | 8 |
| PayeeEligibility | PayeeEligibility | icm | 6 |
| PayeePositionAssignment | PayeePositionAssignment | icm | 6 |
| PayrollExport | PayrollExport | icm | 9 |
| Period | Period | icm | 6 |
| Pipeline | Pipeline | icm | 25 |
| PipelineMapping | PipelineMapping | icm | 13 |
| PipelineRun | PipelineRun | icm | 17 |
| PipelineRunError | PipelineRunError | icm | 10 |
| Plan | Plan | icm | 11 |
| PlanAssignment | PlanAssignment | icm | 8 |
| PlanComponent | PlanComponent | icm | 14 |
| Position | Position | icm | 3 |
| PositionAttribute | PositionAttribute | icm | 6 |
| PositionHierarchy | Position Hierarchy | icm | 5 |
| Quota | Quota | icm | 14 |
| QuotaAllocation | QuotaAllocation | icm | 5 |
| RateTable | RateTable | icm | 10 |
| RateTableBand | RateTableBand | icm | 6 |
| Rule | Rule | icm | 12 |
| SourceConnection | SourceConnection | icm | 8 |
| Statement | Statement | icm | 7 |
| SyncConnection | SyncConnection | icm | 9 |
| Territory | Territory | icm | 3 |
| Title | Title | icm | 8 |
| Transaction | Transaction | icm | 14 |
